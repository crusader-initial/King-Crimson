...
def datetime2timestamp(time_str: str) -> float:
    ...
    Args:
        time_str: TIME_FORMAT格式的日期时间字符串表示
        
    Returns:
        以秒为单位的时间戳
        
    Raises:
        Exception: 如果time_str格式无效
    """
    try:
...
class Note:
    ...
    def __init__(
        ...
        Args:
            noteId: 笔记的唯一标识符
            content: 笔记的文本内容
            createTime: 字符串格式的创建时间戳
            memoryType: 记忆类型（TEXT, MARKDOWN等）
            embedding: 笔记内容的向量表示
            chunks: 笔记被分割成的块列表
            title: 笔记的标题
            summary: 笔记内容的摘要
    ...
    def to_str(self, analysis_type: AnalysisType = None) -> str:
        ...
        Args:
            analysis_type: 用于确定格式的分析类型
            
        Returns:
            笔记的格式化字符串表示
            
        Raises:
            ValueError: 如果memory_type或analysis_type无效
        """
...
class AttributeInfo:
    ...
    def to_str(self):
        # - **[属性名称]**: (属性描述), 置信度级别: [LOW/MEDIUM/HIGH]
